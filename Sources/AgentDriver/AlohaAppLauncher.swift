...
//
// Ported from the shipping CLI's `AlohaAppLauncher`. It lives HERE, next to
// `RemoteAutomationDriver` and `AutomationToken`, because every part of it that is not
// `open(1)` speaks to the app's loopback `AutomationServer`: the readiness probe is one
// `GET /state` over the driver's own transport, which is what presents the bearer
// credential and picks up a rotation. `BrowserTools.AlohaBrowser` — the CDP lane's
// launcher — cannot host it: that target does not link this one, and its wire is
// Chromium's `/json/version`, which knows nothing about windows or headlessness.
//
...
public enum AlohaAppLaunchError: Error, Equatable, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .alreadyRunningHeadless:
            // The shipping CLI names `alohajet quit` here; this package's `quit` closes
...
/// one app, never a second copy. The app claims that URL (`AlohaJetCLISession.attachURLString`
/// on the desktop side) and marks itself as CLI-driven, which is what puts the prompt,
/// the tool trace and the answer in its own AI pane.
public struct AlohaAppLauncher: Sendable {
    ...
    /// The port the app's `AutomationServer` binds (`AlohaJetAutomationController.port`
    /// on the desktop side).
